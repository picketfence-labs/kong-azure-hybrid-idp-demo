...
## 2026-09-16 RDPクリップボード経由で管理端末の`.env`へ保存したADFS client secretが2回とも壊れて保存された

- **何を期待していたか**: Section 3手順4に従い、`.\Register-KongDemoApplication.ps1 -Apply`実行時にクリップボードへ置かれるclient secretを、RDPのクリップボード共有経由で管理端末（macOS）へ渡し、`.env`の`DECK_ADFS_CLIENT_SECRET`へそのまま保存すること。
- **実際どうだったか**: 利用者がVS Code上で`.env`を直接編集して貼り付けたところ、`deck file validate`を`kong/insurance-*.yaml`（`DECK_ADFS_CLIENT_SECRET`を`openid-connect`プラグインの`client_secret`へ`"${{ env "..." }}"`で埋め込む）に対して実行すると`yaml: line N: did not find expected key`で失敗した。値自体は表示せずに構造だけ確認したところ、1回目は460文字・ダブルクォート1個・バックスラッシュ1個、2回目（再度VS Codeへ貼り付け直した後）は449文字・ダブルクォート1個・バックスラッシュ1個・波括弧1個を含んでいた。スクリプトは既存のServer applicationに対しては同じ暗号化済みsecretを読み返すだけで再生成しないため、本来2回とも同一の値になるはずだが、実際には長さも内容も毎回異なっていた。
- **原因の特定**: VS Code等のコードエディタは括弧・クォートの自動補完（bracket/quote auto-closing）を持つため、クリップボードからの貼り付け時にカーソル位置や既存内容との組み合わせによって余分な文字が混入する場合がある。加えて、診断コマンドとして提示した`.env`読み込み方法が当初`source .env`（bashによる評価）だったため、値に含まれる`{`/`}`等の文字がbash構文として解釈され`.env:18: parse error near `}'`のエラーを引き起こした（`.env`はdocker composeの独自パーサーで読まれる想定であり、bashで`source`すべきではない）。
- **対処・解決確認**: (1) 診断はbashの`source`を使わず`grep '^KEY=' .env | cut -d= -f2-`で値を取り出す方式に変更し、値の中身を表示せず長さ・特殊文字の有無だけを確認する手順を確立した。(2) 保存経路からVS Codeでの手打ち・貼り付けを排除し、VM側で`.\Register-KongDemoApplication.ps1 -Apply`を再実行して同じsecretをクリップボードへ再度置いた直後に、管理端末側で`pbpaste`（RDPクリップボード共有でMacのシステムクリップボードにも同じ内容が同期される）を使って直接`.env`へ書き込む方式へ切り替えた。この方式で取得した値は40文字・特殊文字なしの単一行になり、`deck file validate kong/insurance-*.yaml`が全て成功することを確認した。今後、シークレット値を伴う`.env`更新はエディタでの直接貼り付けを避け、`pbpaste`等でクリップボードから直接ファイルへ書き込む方式を優先する。

## 2026-09-16 ADFSのaccess tokenのissuerがdiscoveryのissuerと異なり、openid-connectが401を返す

- **何を期待していたか**: Firefoxで`http://localhost:8000/adfs/auth/start`から`demo-claim@adfsdemo.picketfencelabs.local`でサインインし、`http://localhost:8000/adfs/auth/callback`でセッションが確立されること（Section 4/5）。
- **実際どうだったか**: ADFSでのサインイン自体は成功し`/adfs/auth/callback`へリダイレクトされたが、`{"message":"Unauthorized"}`（HTTP 401）が返った。
- **原因の特定**: `docker compose logs kong`を確認したところ、次のログが出ていた: `[openid-connect] invalid issuer (http://adfs.adfsdemo.picketfencelabs.local/adfs/services/trust) was specified for access token, https://adfs.adfsdemo.picketfencelabs.local/adfs was expected`。ADFSはid_tokenのissuerにはdiscovery文書通りの`https://<FQDN>/adfs`を使うが、access tokenのissuer（`iss`claim）にはWS-Trust由来のレガシー値`http://<FQDN>/adfs/services/trust`を使うという既知の仕様差がある。`openid-connect`プラグインの`config.issuer`は単一値の期待issuerとしてのみ機能し、この差分を許容しないため401になっていた。`kong/insurance-product.yaml`等の冒頭コメントに元々あった「要検証: ADFSのOIDC issuer discovery URLの実際の形式」という懸念点が的中した形。
- **対処・解決確認**: `openid-connect`プラグインのschema（`GET /schemas/plugins/openid-connect`）を確認したところ、`issuers_allowed`（`iss` claimとして許容する値の配列）というフィールドが存在した。ADFSと通信する全Route（`kong/insurance-ui-route.yaml`の`insurance-adfs-auth-login`、および`kong/insurance-{product,application,claim,customer,policy,simulation}.yaml`の計7箇所）の`openid-connect`設定へ、`issuers_allowed: [DECK_ADFS_ISSUER, "http://adfs.adfsdemo.picketfencelabs.local/adfs/services/trust"]`を追加した。`deck gateway diff`で7件のplugin更新のみ（作成・削除なし）であることを確認した上で`deck gateway sync`を実行し反映した（利用者承認済み）。session-onlyの`insurance-adfs-auth-session`/`insurance-adfs-authz-probe`はIdPへ再度トークンを問い合わせないため対象外とした。実機での再ログイン確認は次のステップで実施する。

## 2026-09-17 アプリ側「この経路だけログアウト」後の再ログインが、ADFS側SSOセッション残存により`/adfs/auth/callback`で401になる

- **何を期待していたか**: Playwright（Chromium）でinsurance-uiのADFSログイン権限差テストを行うにあたり、1ユーザーでログイン→APIボタンで許可/拒否を確認→アプリの「この経路だけログアウト」ボタンでログアウト→ページreload→次のユーザーで再度「別画面でログイン」、という手順で、毎回ADFSのサインインフォーム（ユーザー名/パスワード入力）が表示されること。
- **実際どうだったか**: 1人目（`demo-sales`）は正常にサインインフォームが表示され、ログイン・API呼び出し（Application API許可/Claim API拒否）とも想定通りだった。アプリの「この経路だけログアウト」でログアウトし、ページをreloadして2人目（`demo-policy-admin`）で「別画面でログイン」をクリックしたところ、ポップアップはサインインフォームを一切表示せず、いきなり`http://localhost:8000/adfs/auth/callback`（`{"message":"Unauthorized"}`、HTTP 401）に着地した。ポップアップを閉じて再度同じ手順を試しても同じ401が再現した（アプリ側のメイン画面はADFSカードが「signed out」のままだった）。
- **原因の特定**: アプリの「この経路だけログアウト」はアプリ側（Kongの認証セッション/state）のみをクリアし、ADFS自体が保持するブラウザ側のSSOセッションクッキーはクリアしない。そのため2人目の「別画面でログイン」クリック時、ADFSは前回ユーザー（`demo-sales`）のSSOセッションを検出して対話的ログインフォームを出さずにサイレントに認可コードへ進んだが、この経路だと（要調査だが）Kong側のstate/nonce検証もしくはセッションクッキーとの不整合により401になった模様（直前の2026-09-16エントリの「issuerの不一致」とは別原因。issuerの修正は当時のセッションで既に反映済みの状態でこの401が再現しているため）。ブラウザで`https://adfs.adfsdemo.picketfencelabs.local/adfs/oauth2/logout`へ直接アクセスするとADFSは「正常にサインアウトしました」と表示し、この状態でアプリへ戻って再度ログインを試みると、毎回確実にサインインフォームが表示されることを確認した（3人目`demo-it`・4人目`demo-new-business`でも同じ手順で再現性あり）。
- **対処・回避方法**: ユーザーを切り替えてログインし直す際は、アプリの「この経路だけログアウト」だけでなく、**ADFS自身の`/adfs/oauth2/logout`エンドポイントに一度アクセスしてSSOセッションを明示的に破棄してから**次のユーザーでログインする。今回のPlaywright手動テストではこの2段階ログアウト（アプリ側→ADFS側）を徹底することで、4ユーザー・6ボタンクリック全てで想定通りのサインインフォーム表示とAPI許可/拒否結果を得られた。恒久対応（アプリの「この経路だけログアウト」ボタン自体にADFSのRP-initiated logout、または`prompt=login`パラメータを付与する等）は本タスクのスコープ外のため未着手。将来、複数ユーザーが同一ブラウザで頻繁に切り替わる運用（デモの複数回実施等）が想定される場合は、UI側の対応を検討する価値がある。

## 2026-09-17 Group 1（Entra ID）回帰テストで、`inquiry_only`/`both_apis`ユーザーがMFA登録画面でブロックされる

- **何を期待していたか**: TESTING.md「Group 1: Chat UI」節のシナリオ②③を回帰実施するにあたり、`demo-inquiry-only@hashipicketfence.onmicrosoft.com`・`demo-both-apis@...`でメール・パスワードのみでサインインが完了し、Chat UIへ遷移すること（シナリオ①の`demo-no-agent-access`同様、パスワード入力後すぐに結果が出ることを想定）。
